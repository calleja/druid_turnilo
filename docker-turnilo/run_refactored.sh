...
if [ -n "$CONFIG_FILE" ]; then
  ...
  echo "About to execute: turnilo run-config /etc/config/turnilo/config.yml"
  ...
  turnilo run-config /etc/config/turnilo/config.yml
elif [ -n "$MYARGS" ]; then
  ...
  if [ -n "$DUMP_CONFIG" ]; then
    ...
    echo "About to execute: turnilo introspect-druid ${DRUID_BROKER_URL} > /etc/config/export/new-config.yml"
    turnilo introspect-druid ${DRUID_BROKER_URL} > /etc/config/export/new-config.yml
  else
    echo "Arguments provided: DRUID_BROKER_URL = $DRUID_BROKER_URL"
    # User is supplying broker URL
    echo "About to execute: turnilo connect-druid ${DRUID_BROKER_URL}"
  ...
  echo "About to execute: turnilo connect-druid http://druid_host:8082"
  turnilo connect-druid http://druid_host:8082
fi